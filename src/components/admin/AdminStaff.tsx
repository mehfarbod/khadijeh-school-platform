...
const isTeacherPosition = (position: string) => {
  const value = position.trim().toLowerCase();
  return (
    value.includes("معلم") ||
    value.includes("دبیر") ||
    value.includes("مدرس") ||
    value.includes("آموزگار")
  );
};
...
export default function AdminStaff() {
  ...
  const loadStaff = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("خطا در دریافت اطلاعات کادر");
      }
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "خطا در دریافت اطلاعات کادر مدرسه"
      );
    }
  };
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("خطا در دریافت حساب‌های پنل");
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        editId ? "اطلاعات کادر ویرایش شد" : "عضو جدید به کادر اضافه شد"
      );
  ...
  const toggleActive = async (member: Staff) => {
    try {
      ...
      toast.success(
        member.isActive ? "عضو کادر غیرفعال شد" : "عضو کادر فعال شد"
      );
      await loadStaff();
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "خطا در تغییر وضعیت"
      );
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "خطا در حذف کادر"
      );
    }
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              مدیریت اطلاعات کادر آموزشی و اداری مدرسه
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative max-w-md flex-1">
            ...
            <Input
              placeholder="جستجو بر اساس نام، سمت، دسته‌بندی، درس..."
              value={search}
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#DBE7C1] text-[#194342]">
                            {member.photo ? (
                      ...
                      <td className="px-5 py-4">
                        {member.user ? (
                          <div className="flex items-center gap-2 text-xs">
                            <Link2 className="h-3.5 w-3.5 text-[#194342]" />
                            <div>
                        ...
                        >
                          {member.isActive ? (
                            <>
                              ...
                              <span className="text-green-700">فعال</span>
                            </>
                          ) : (
                            <>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto" dir="rtl">
          <DialogHeader>
            <DialogTitle>
              {editId ? "ویرایش عضو کادر" : "افزودن عضو کادر"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6 py-2">
            <section>
              <div className="mb-4 flex items-center gap-2">
                <div className="rounded-lg bg-[#F1F5E8] p-2 text-[#194342]">
                  <UserRound className="h-4 w-4" />
                </div>
                <div>
                  <h2 className="text-sm font-semibold">اطلاعات اصلی</h2>
                  <p className="text-xs text-muted-foreground">
                    اطلاعات پایه عضو کادر مدرسه
                  </p>
                </div>
              </div>
                ...
                {teacher && (
                  <div className="sm:col-span-2">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      این فیلد برای معلم، دبیر، مدرس یا آموزگار نمایش داده می‌شود.
                    </p>
                  </div>
                )}
              </div>
            ...
            <section>
              <div className="mb-4 flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-semibold">اطلاعات تماس</h2>
                  <p className="text-xs text-muted-foreground">
                    اطلاعات تماس در صورت نیاز قابل ثبت است.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <Input
                    value={form.photo}
                    onChange={(event) => updateForm("photo", event.target.value)}
                    className="mt-1.5"
                    placeholder="https://..."
                    dir="ltr"
                  />
                  <p className="mt-1 text-xs text-muted-foreground">
                    فعلاً آدرس تصویر ثبت می‌شود؛ آپلود فایل را می‌توانیم بعداً به سیستم اضافه کنیم.
                  </p>
                </div>
              </div>
            </section>

            <section className="rounded-xl border border-[#DBE7C1] bg-[#F1F5E8]/50 p-4">
              <div className="mb-3 flex items-center gap-2">
                <Link2 className="h-4 w-4 text-[#194342]" />
                <div>
                  <h2 className="text-sm font-semibold">اتصال به حساب پنل</h2>
                  <p className="text-xs text-muted-foreground">
                    اختیاری است؛ هر عضو کادر لزوماً کاربر پنل نیست.
                  </p>
                </div>
              </div>
              ...
              >
                <option value="">
                  {loadingUsers ? "در حال دریافت حساب‌ها..." : "بدون حساب پنل"}
                </option>
                {users
                  .filter((user) => {
                    if (user.id === form.userId) return true;
                    return !staff?.some((member) => member.userId === user.id);
                  })
                  .map((user) => (
                    <option key={user.id} value={user.id}>
                      {(user.name || "بدون نام")} — {user.email} —{" "}
            ...
            <section>
              <div className="flex items-center justify-between rounded-xl border border-border/60 p-4">
                <div>
                  <p className="text-sm font-medium">وضعیت عضو کادر</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    عضو غیرفعال در لیست‌های عمومی نمایش داده نمی‌شود.
                  </p>
                </div>
                ...
                <button
                  ...
                  className={`relative h-6 w-11 rounded-full transition ${
                    form.isActive ? "bg-[#194342]" : "bg-muted"
                  }`}
                  aria-pressed={form.isActive}
                >
                  <span
                    className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${
                      form.isActive ? "right-1" : "right-6"
                    }`}
                  />
                </button>
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={saving}>
              {saving ? "در حال ذخیره..." : editId ? "ذخیره تغییرات" : "افزودن عضو"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            <AlertDialogTitle>حذف عضو کادر</AlertDialogTitle>
            <AlertDialogDescription>
              با حذف این عضو، اطلاعات او از کادر مدرسه حذف می‌شود. این عملیات قابل بازگشت نیست.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
